...
async def create_cardio(cardio: CardioSession, user=Depends(get_current_user)):
    ...
    result = await db.cardio.insert_one(cardio.dict(by_alias=True, exclude_none=True))
    return str(result.inserted_id)

@router.get("/", response_model=List[CardioSession])
async def get_cardio(user=Depends(get_current_user)):
    sessions = []
    cursor = db.cardio.find({"user_id": str(user["_id"])}).sort("date", -1)
    async for document in cursor:
        sessions.append(CardioSession(**document))
    return sessions
